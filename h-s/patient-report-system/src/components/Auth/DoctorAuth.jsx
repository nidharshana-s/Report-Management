...
const DoctorAuth = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      await axios.post("/api/doctor/send-otp", { phno: formData.phno });
      setOtpSent(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        await axios.post("/api/doctor/login", { phno: formData.phno, otp: formData.otp });
        navigate("/doctor-dashboard");
      } else {
        await axios.post("/api/doctor/register", {
          name: formData.name,
          age: formData.age,
  ...
  return (
    <div className="auth-container">
      <div className="auth-box animate-slide-in">
        <h2 className="auth-title">{isLogin ? "Doctor Login" : "Doctor Registration"}</h2>
